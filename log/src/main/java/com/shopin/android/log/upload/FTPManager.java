...
/**
 * @author will on 2018/1/24 14:49
 * @email
 *  pengweiqiang64@163.com
 * @description
 *  FTP上传下载管理
 * @version
 *  1.0.1
...
public class FTPManager {
    ...
    // 连接到ftp服务器
    public synchronized boolean connect(String serverPath) throws Exception {
        boolean bool = false;
        if (ftpClient.isConnected()) {//判断是否已登陆
            ftpClient.disconnect();
        }
        ftpClient.setDataTimeout(10000);//设置连接超时时间
    ...
    public synchronized boolean uploadFile(String localPath, String serverPath)
        ...
        if (!localFile.exists()) {
            System.out.println(localPath+"本地文件不存在");
            return false;
        }
...
//        createDirectory(serverPath); // 如果文件夹不存在，创建文件夹
        ...
        long serverSize = 0;
        if (files.length == 0) {
            System.out.println(serverPath+"服务器文件不存在");
            serverSize = 0;
        } else {
            ...
            if (currentSize / step != process) {
                ...
                if (process % 10 == 0) {
                    if(uploadProgressListener!=null){
                        uploadProgressListener.onUploadProgress(localPath,process);
                    }
    ...
    public interface UploadProgressListener {
        public void onUploadProgress(String currentStep, long process);
